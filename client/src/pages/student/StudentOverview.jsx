...
import api from '../../api';
import { BookOpen, BarChart2, CreditCard, Bell } from 'lucide-react';
...
const StudentOverview = () => {
  ...
  const statCards = [
    { label: 'Enrolled Courses', value: stats?.ongoingCourses ?? 0, icon: BookOpen, color: '#4f46e5', bg: '#ede9fe' },
    { label: 'Attendance Rate', value: `${stats?.attendancePercentage ?? 100}%`, icon: BarChart2, color: stats?.attendancePercentage >= 75 ? '#059669' : '#dc2626', bg: stats?.attendancePercentage >= 75 ? '#d1fae5' : '#fee2e2' },
    { label: 'Total Paid Fees', value: `₹${(stats?.totalPaidFees || 0).toLocaleString('en-IN')}`, icon: CreditCard, color: '#d97706', bg: '#fef3c7' },
  ];
            ...
            {stats.schedule.map(entry => (
              <div key={entry._id} className="flex items-center gap-4 p-5 hover:bg-gray-50">
                <div className="px-3 py-2 rounded-xl text-center" style={{ background: '#ede9fe' }}>
                  <p className="text-xs font-semibold" style={{ color: '#6d28d9' }}>{entry.day}</p>
                  <p className="text-xs" style={{ color: '#7c3aed' }}>{entry.startTime} – {entry.endTime}</p>
                </div>
                <div>
