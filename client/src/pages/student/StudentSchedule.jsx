    ...
    >
      <motion.div variants={itemVariants} className="flex justify-between items-end">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-brand-500 uppercase tracking-[0.3em]">Learning Timeline</span>
          </div>
          <h2 className="text-4xl font-bold text-slate-900 font-serif tracking-tight">Your Class Schedule</h2>
          <p className="text-slate-500 font-medium mt-1 italic">Mastery is built session by session.</p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 space-y-4">
          ...
          <p className="text-slate-400 font-medium animate-pulse font-serif italic">Drawing your path...</p>
        </div>
      ) : timetable.length === 0 ? (
        <motion.div variants={itemVariants} className="bento-card py-24 flex flex-col items-center justify-center text-center space-y-6 bg-slate-50/30 border-dashed">
          ...
          <div>
            <h3 className="text-2xl font-bold text-slate-900 font-serif">A Blank Canvas</h3>
            <p className="text-slate-500 mt-2 max-w-xs mx-auto">You don't have any classes scheduled at the moment. Explore our courses to begin.</p>
          </div>
        </motion.div>
      ) : (
            ...
            return (
              <motion.div key={day} variants={itemVariants} className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="px-6 py-2 rounded-full bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em]">
                    {day}
                  </div>
                        ...
                        <div className="flex items-start gap-8 relative z-10">
                          ...
                          <div className="flex-1 pt-2">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <h4 className="text-2xl font-bold text-slate-900 font-serif tracking-tight group-hover:text-brand-600 transition-colors">{entry.course?.name || 'Class Session'}</h4>
                            </div>
                            ...
                            <div className="space-y-3 mt-6">
                              ...
                              {entry.assignedTeacher && (
                                <div className="flex items-center gap-2 text-slate-400">
                                  ...
                                  <span className="text-xs font-bold uppercase tracking-widest">Mentor: {entry.assignedTeacher.name}</span>
                                </div>
                              )}
                            </div>
                          </div>
